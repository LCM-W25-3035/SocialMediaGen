...
api_key_sources = {api_keys[0]: {'technology': 'techcrunch', 'sports': 'espn', 'entertainment': 'buzzfeed', 'politics': 'cnn', 'business': 'business-insider'},
                ...
                try:
                    ...
                    if response.get("articles"):
                        for article in response["articles"]:
                            ...
                            await producer.send(KAFKA_TOPIC, article)
                            print(f"Produced: {article['title']} (Category: {category}, Source: {source})")
...
async def consume_and_store():
    ...
    try:
        async for message in consumer:
            ...
            if not await collection.find_one({"url": news_article.get("url")}):
